...
import ExprienceOneComponent from "../ModelComponents/exprienceOneComponent";
  ...
  const handleSubmit = async (e) => {
    ...
    const res = await database
      .updateDocument(
        ...
        userData?.$id,
        {
          exprience: JSON.stringify({ exprience: experience }),
        }
      )
      .then((res) => {
        closeModal();
          ...
          {userData?.exprience &&
            JSON.parse(userData.exprience).exprience.map((item, index) => (
              <div key={index} className="flex items-center gap-[10px]">
                <div>
                  <Image
                    src={`https://res.cloudinary.com/dfk5jbk5r/image/upload/v1685449092/TalentBolt/Frame_1_nq5oz2.png`}
                    width={73}
                    height={73}
                    alt="logo"
                  />
                ...
                <div className="flex flex-col min-w-[250px]">
                  ...
                  <p className="text-[14px] text-[#111111]/80">{item.level}</p>
        ...
        <div className="w-[717px] h-[550px] overflow-y-scroll">
          ...
          <div className="px-[25px] py-[20px] flex flex-col gap-[30px]">
            {experience.map((item, index) => {
              return (
                <ExprienceOneComponent key={item.id}  userData={userData} item={item} index={index} experience={experience} setExprience={setExprience} />
              );
            })}
